...
- `<body data-ocean>` があるページで、`js/ocean/boot.js` が端末を見て本体を落とすか決める。
  本体は画面いっぱいの固定canvas（`.page-water`・z-index 1）を1枚張り、2つの板を描く。
...
- **水面は絵ではなく計算**。2次元の波動方程式をコンピュートシェーダで解いている
  （`ocean-src/src/ripple.js`）。カーソルとスクロールが水を押すと、波が立ち、伝わり、
  端で跳ね返り、干渉して収まる。WebGPUが無ければ解析的な近似に落ちる。
...
- 写真の中の水平線は起動時に画素から実測する。海と人物の切り分けは青緑の強さ
  （リニア空間で 空/海 0.25以上・人/岩 0.17以下）。写真を差し替えても追随する。
...
- 走り出したあともフレーム時間を見て解像度を自動で落とす。タブが裏、または水が静まって
  ヒーローも画面に無いときは描画頻度を半分に落とす。
- 開発用: **Shift+O** で計測パネル。`window.__ocean.ripple.params` /
  `.heroPhoto.params` / `.wash.params` の `.value` を書き換えると即座に効く。
